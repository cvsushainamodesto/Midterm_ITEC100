...
<body>
  ...
  <fieldset class="register_box">
      <center><label style="font-style: bold; color: green; position: absolute; top: 20%; left: 44%;">
      REGISTRATION FORM</label></center> <br>
  ...
  <div id="modal_message" class="modal_message">
    <div class="modal_message_content">
      ...
      <div class="modal_message_footer">
        <form action="add_account.php" method="post">
          <center><button class="ok" id="ok" name="ok">OKAY</button></center>
          </form>
       </div>
    </div>
  </div>
  ...
  if (isset($_SESSION['success']) && $_SESSION['success'] == 'true') {
    echo '<script>show_message("Registered Successfully!","block");</script>'; 
    $_SESSION['success'] = 'false';
  }
  // DISPLAY ERROR MSG
  else if (isset($_SESSION['error']) && $_SESSION['error'] == 'true') {
    echo '<script>show_message("Connection Error!","block");</script>'; 
    $_SESSION['error'] = 'false';
  }
  else if (isset($_SESSION['user_exist']) && $_SESSION['user_exist'] == 'true') {
    echo '<script>show_message("Username is already existing.","block");</script>'; 
    $_SESSION['user_exist'] = 'false';
  }
  else if (isset($_SESSION['email_exist']) && $_SESSION['email_exist'] == 'true') {
    echo '<script>show_message("Email is already existing.","block");</script>'; 
